...
class Semanticizer:
    ...
    def semanticize(self, sentence, normalize_dash=True,
        ...
        for ngram in ngrams:
            ...
            if rds.exists('norm:' + normal_ngram):
                normalized_ngram = self.normalize(ngram, normalize_dash,
                                                  normalize_accents,
                                                  normalize_lower)
                for anchor in rds.smembers('norm:' + normal_ngram):
                    ...
                    if normalized_ngram == normalized_anchor:
                        if not rds.exists('txt:' + anchor):
                            raise LookupError("Something went wrong, cannot "
                                              + "find %s in the database" \
                                              % anchor)
                        label_data = rds.lrange('txt:' + anchor, 0, -1)
                        if len(label_data) < 4:
                            continue
                        ...
                        for sense in sense_ids:
                            ...
                            if label[2] == 0:
                                ...
                                # used
                                sense_probability = float(sense_data[1]) \
                                                    / label[3]
                            if sense_probability > sense_probability_threshold:
                                ...
                                if counts:
                                    ...
                                    link["senseOccCount"] = sense_data[0]
                                    link["senseDocCount"] = sense_data[1]
                                    link['fromTitle'] = sense_data[2]
                                    link['fromRedirect'] = sense_data[3]
